...
# ==========================================
# TASK 4A: CONVERGENCE VISUALIZATION
# ==========================================
def plot_optimizer_convergence(data_dict):
    """Plots the optimization trajectory of our classical optimizers."""
    print("\n=== [TASK 4A] GENERATING CONVERGENCE PLOT ===")
    
    plt.figure(figsize=(10, 6))
    plt.style.use('dark_background') 
    
    colors = {'COBYLA': '#00FFAA', 'Nelder-Mead': '#FF00AA', 'SPSA': '#00AAFF'}
    
    for opt_name, data in data_dict.items():
        history = data["History"]
        iterations = range(len(history))
        plt.plot(iterations, history, label=f"{opt_name} (Final: {data['Final Energy']:.2f})", 
                 color=colors.get(opt_name, 'white'), linewidth=2)
        
    plt.title("QAOA Optimizer Convergence ($p=2$)", fontsize=16, pad=15)
    ...
    plt.savefig("qaoa_convergence_plot.png", dpi=300)
    print("Task 4A Complete: Saved as 'qaoa_convergence_plot.png'")
    plt.show() # PROGRAM WILL PAUSE HERE UNTIL YOU CLOSE THE WINDOW

# ==========================================
# TASK 4B: HEATMAP & OPTIMIZATION PATH
# ==========================================
def generate_heatmap_and_path(cost_hamiltonian, mixer_hamiltonian, p_layer=1):
    """Generates a QAOA Cost Landscape Heatmap and tracks COBYLA's path."""
    print("\n=== [TASK 4B] GENERATING HEATMAP & PATH ===")
    print("Initializing Quantum Heatmap Scanner... (This will take ~30-60 seconds)")
    ...
    def objective(params):
        path_gamma.append(params[0]) 
        path_beta.append(params[1])  
    ...
    minimize(objective, initial_params, method='COBYLA', options={'maxiter': 50})
    ...
    for i, g in enumerate(gamma_vals):
        for j, b in enumerate(beta_vals):
            pub = (circuit, cost_hamiltonian, [[g, b]])
            energy_grid[j][i] = float(estimator.run([pub]).result()[0].data.evs[0])
            
    plt.figure(figsize=(10, 8))
    
    contour = plt.contourf(gamma_vals, beta_vals, energy_grid, levels=50, cmap='viridis')
    plt.colorbar(contour, label='Expectation Value (Energy)')
    
    plt.plot(path_gamma, path_beta, color='white', marker='o', linestyle='-', 
             linewidth=2, markersize=4, label='COBYLA Path')
    
    plt.scatter(path_gamma[0], path_beta[0], color='red', s=100, label='Start', zorder=5)
    plt.scatter(path_gamma[-1], path_beta[-1], color='cyan', s=100, label='Finish', zorder=5)

    plt.title("QAOA Cost Landscape & Optimization Path (p=1)")
    plt.xlabel("Gamma (Cost Angle)")
    plt.ylabel("Beta (Mixer Angle)")
    plt.legend()
    
    plt.savefig("qaoa_heatmap_path.png", dpi=300)
    print("Task 4B Complete: Saved as 'qaoa_heatmap_path.png'")
    plt.show()
...
if __name__ == "__main__":
    ...
    # 2. Run Task 3 (Benchmark)
    benchmark_data = benchmark_optimizers(p_layer=2)
    
    # 3. Run Task 4A (Convergence Plot)
    plot_optimizer_convergence(benchmark_data)
    
    # 4. Run Task 4B (Heatmap & Path)
    generate_heatmap_and_path(cost_hamiltonian, mixer_hamiltonian, p_layer=1)
